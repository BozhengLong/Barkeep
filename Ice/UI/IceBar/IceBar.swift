            ...
            window.publisher(for: \.frame)
                ...
                .sink { [weak self, weak window] _ in
                    guard
                        ...
                        let info = window
                            .flatMap({ CGWindowID(safeWindowNumber: $0.windowNumber) })
...
private struct IceBarContentView: View {
    ...
    /// The color the bar tints itself with. Prefer the Ice Bar's own sampled color;
    /// on macOS 26 that sampling can return nil, so fall back to the menu bar
    /// manager's average color (which the settings Layout pane uses successfully).
    private var barColorInfo: MenuBarAverageColorInfo? {
        colorManager.colorInfo ?? menuBarManager.averageColorInfo
    }
    ...
    var body: some View {
        ZStack {
            content
                ...
                .layoutBarStyle(appState: appState, averageColorInfo: barColorInfo)
                .foregroundStyle(barColorInfo?.color.brightness ?? 0 > 0.67 ? .black : .white)
                .clipShape(clipShape)
