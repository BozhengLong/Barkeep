...
final class MenuBarItemImageCache: ObservableObject {
    ...
    func createImages(for section: MenuBarSection.Name, screen: NSScreen) async -> [CGWindowID: CGImage] {
        ...
        // Keyed by windowID (unique per item this session). On macOS 26 the menu bar is
        // consolidated under Control Center, so many items share the same MenuBarItemInfo
        // (namespace com.apple.controlcenter, title "Item-0"); keying images by info would
        // collide them into one slot and leave most items showing a placeholder.
            ...
            CGFloat(compositeImage.width) == frame.width * backingScaleFactor
        {
            for windowID in windowIDs {
                ...
                guard let itemImage = compositeImage.cropping(to: cropFrame) else {
                    continue
                }
            ...
            for windowID in windowIDs {
                ...
                guard
                    ...
                    let croppedImage = itemImage.cropping(to: cropFrame)
                else {
                    continue
                }
