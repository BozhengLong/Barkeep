...
private extension MenuBarItemInfo {
    ...
    init(uncheckedItemWindow itemWindow: WindowInfo) {
        ...
        self.title = title
        // On macOS 26 (Tahoe) the menu bar is consolidated under Control Center, so
        // Barkeep's own control items report `com.apple.controlcenter` as their owner
        // instead of Barkeep. Recognize our control items by their unique title
        // (the status item's autosaveName: SItem/HItem/AHItem) and restore the Ice
        // namespace, so control-item matching (which keys on namespace == .ice) keeps
        // working. Without this, Ice can't find its dividers and clears the item cache.
